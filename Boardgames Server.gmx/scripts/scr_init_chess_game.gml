...
with(obj_player) if(gameType=other.gameType) writestring(myName);
send_client("all");

if(global.chessPlayers<=2) { //the two competitors
clearbuffer();
...
if(global.chessPlayers=2) { //last player to connect
clearbuffer();
writebyte(setturn);
writebyte(0);
