    ...
    switch(readbyte()) 
    {
        case movement:
            switch(readstring()) 
            {
                case "obj_pawn":
                    var ID;
                    ID=readushort();
                    for(i=0;i<instance_number(obj_pawn);i+=1) //walk through pawns
                    {
                        if(ID=instance_find(obj_pawn,i).ID) //this is the pawn you're looking for
                        {
                            if(instance_find(obj_pawn,i).image_single=1) //pivot: looking for white
                            {
                                with(instance_find(obj_pawn,i)) //for each pawn that is the only one you're looking for
                                {
                                    x+=readshort()*-1;
                                    y+=readshort()*-1;
                                    if(collision_point(x,y,obj_pawn,0,1)) //if this is killing a pawn
                                        with(collision_point(x,y,obj_pawn,0,1)) //for each pawn that is getting killed
                                        { 
                                            x=30; y=420; //move to the dead pile
                                            while(collision_point(x,y,all,0,1)) //if there is already a dead piece
                                                x+=20; //bump it to the right
                                        }
                                    if(collision_point(x,y,obj_knight,0,1)) 
                                        with(collision_point(x,y,obj_knight,0,1)) 
                                        { 
                                            x=30; y=420; 
                                            while(collision_point(x,y,all,0,1)) 
                                                x+=20; 
                                        }
                                    if(collision_point(x,y,obj_rook,0,1)) 
                                        with(collision_point(x,y,obj_rook,0,1)) 
                                        { 
                                            x=30; y=420; 
                                                while(collision_point(x,y,all,0,1)) 
                                                    x+=20; 
                                    ...
                                    {
                                        with(collision_point(x,y,obj_king,0,1)) 
                                        { 
                                            x=30; 
                                            y=420; 
                                            while(collision_point(x,y,all,0,1)) 
                ...
                case "obj_knight":
                    ...
                    for(i=0;i<instance_number(obj_knight);i+=1)
                        if(ID=instance_find(obj_knight,i).ID) {
                            if(instance_find(obj_knight,i).image_single=1) //pivot: looking for white
                            {
                ...
                case "obj_rook":
                    ...
                    for(i=0;i<instance_number(obj_rook);i+=1)
                        if(ID=instance_find(obj_rook,i).ID) {
                            if(instance_find(obj_rook,i).image_single=1)  //pivot: looking for white
                            {
                ...
                case "obj_bishop":
                    ...
                    for(i=0;i<instance_number(obj_bishop);i+=1)
                        if(ID=instance_find(obj_bishop,i).ID) {
                            if(instance_find(obj_bishop,i).image_single=1)  //pivot: looking for white
                            {
                ...
                case "obj_queen":
                    ...
                    for(i=0;i<instance_number(obj_queen);i+=1)
                        if(ID=instance_find(obj_queen,i).ID) {
                            if(instance_find(obj_queen,i).image_single=1)  //pivot: looking for white
                            {
                ...
                case "obj_king":
                    ...
                    for(i=0;i<instance_number(obj_king);i+=1)
                        if(ID=instance_find(obj_king,i).ID) {
                            if(instance_find(obj_king,i).image_single=1)  //pivot: looking for white
                            {
        ...
        case specmovement:
            switch(readstring()) {
                ...
                if(instance_find(obj_pawn,i).image_single=myImageSingle)  //pivot: looking for white?
                {
                ...
                for(i=0;i<instance_number(obj_rook);i+=1)
                if(ID=instance_find(obj_rook,i).ID) {
                if(instance_find(obj_rook,i).image_single=myImageSingle)  //pivot: looking for white?
                {
                    with(instance_find(obj_rook,i)) {
                        if(myImageSingle=1)   //pivot: looking for white
                        {
                            x+=readshort()*-1;
                            y+=readshort()*-1;
                        }
                ...
                for(i=0;i<instance_number(obj_knight);i+=1)
                if(ID=instance_find(obj_knight,i).ID) {
                if(instance_find(obj_knight,i).image_single=myImageSingle)   //pivot: looking for white?
                {
                    with(instance_find(obj_knight,i))   //pivot: looking for white
                    {
                        if(myImageSingle=1) {
                            x+=readshort()*-1;
                            y+=readshort()*-1;
                ...
                for(i=0;i<instance_number(obj_bishop);i+=1)
                if(ID=instance_find(obj_bishop,i).ID) {
                if(instance_find(obj_bishop,i).image_single=myImageSingle)   //pivot: looking for white?
                {
                    with(instance_find(obj_bishop,i)) {
                        if(myImageSingle=1)   //pivot: looking for white
                        {
        ...
        case setopponent:
            //if(global.gameHasBegun=0) 
                scr_chat_write("System: " + readstring() + " is your opponent.",c_teal);
            //scr_clear_pieces();
            //scr_reset_pieces(); //pivot: this creates all the "chess pieces"
            with(obj_cell) 
                event_user(0);
        ...
        case setindex: //pivot: added to set player index
            global.playerIndex = readshort();
            scr_clear_pieces();
            scr_reset_pieces(); //pivot: this creates all the "chess pieces"
            break;
        ...
        case getplayers: //pivot: global.playerNames[global.connectedClients-1] is not recieving "readstring()" here
            global.connectedClients=readbyte();
            for(i=0;i<global.connectedClients;i+=1)
                global.playerNames[i]=readstring();
            break;
